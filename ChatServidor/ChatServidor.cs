...
namespace ChatServidor
{
    ...
    class ChatServidor
    {
        ...
        public static Hashtable htUsuarios = new Hashtable(2); // 2 usuarios é o limite definido
        // Esta hash table armazena os usuários e as conexões (acessada/consultada por conexão)
        public static Hashtable htConexoes = new Hashtable(2); // 2 usuários é o limite definido
        ...
        public static void AddUser(TcpClient tcpUsuario, string strUsername)
        {
            ...
            SendAdmMsg(htConexoes[tcpUsuario] + " entrou..");
        }
        ...
        public static void RemoveUser(TcpClient tcpUsuario)
        {
            ...
            if (htConexoes[tcpUsuario] != null)
            {
                ...
                SendAdmMsg(htConexoes[tcpUsuario] + " saiu...");
        ...
        public static void SendAdmMsg(string Mensagem)
        {
    ...
    class Conexao
    {
        ...
        private void AceitaCliente()
        {
            ...
            actualUser = srReceptor.ReadLine();

            // temos uma resposta do cliente
            if (actualUser != "")
            {
                // Armazena o nome do usuário na hash table
                if (ChatServidor.htUsuarios.Contains(actualUser) == true)
                {
                ...
                else if (actualUser == "Administrator")
                {
                    // 0 => não conectado
                ...
                {
                    // 0 => não conectado
                    swEnviador.WriteLine("0|Nome de usuário vazio");
                    swEnviador.Flush();
                    CloseConnection();
                    return;
                }
                else
                {
                    ...
                    ChatServidor.AddUser(tcpCliente, actualUser);
                }
            }
            else
            {
            ...
            try
            {
                ...
                while ((strResposta = srReceptor.ReadLine()) != "")
                {
                    ...
                    if (strResposta == null)
                    {
                        ChatServidor.RemoveUser(tcpCliente);
                    }
                    else
                    {
                        // envia a mensagem para todos os outros usuários
                        ChatServidor.EnviaMensagem(actualUser, strResposta);
                    }
                }
            }
            catch
